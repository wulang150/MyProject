...
typedef NS_ENUM(NSInteger, ZYNetworkType) {
    ...
    ZYNetworkTypeWiFi    ,          //wifi
    ZYNetworkTypeCellularData ,     //数据流量
};
...
- (void)startCheck {
    ...
    __block ZYNetworkAccessibleState netState = ZYNetworkUnknown;
    //判断网络类型
    [self getCurrentNetworkType:^(ZYNetworkType type) {
        if (type == ZYNetworkTypeWiFi){
            netState = ZYNetworkWIFI;
        }
        else if (type == ZYNetworkTypeCellularData){
            netState = ZYNetworkWWAN;
        }
        ...
        if ([UIDevice currentDevice].systemVersion.floatValue < 10.0 || [self currentReachable]) {
             ...
             **/
            [self notiWithAccessibleState:netState];
            return;
        }
        ...
        switch (state) {
            case kCTCellularDataRestricted: {// 系统 API 返回 无蜂窝数据访问权限
                //在只开wifi权限，或者关闭权限，都会走这里
                //这里是除了开启了wifi但网络不通，无法判断是否为权限关闭问题
//                if (netState == ZYNetworkWIFI){
