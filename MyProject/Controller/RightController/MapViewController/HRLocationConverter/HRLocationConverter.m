...
@implementation HRLocationConverter
...
{
    long double z = sqrt(coordinate.longitude * coordinate.longitude + coordinate.latitude * coordinate.latitude) + 0.00002 * sin(coordinate.latitude * xPi);
    long double theta = atan2(coordinate.latitude, coordinate.longitude) + 0.000003 * cos(coordinate.longitude * xPi);
...
+(CLLocationCoordinate2D)transformFromGCJToWGS:(CLLocationCoordinate2D)coordinate
{
    ...
    while(true)
    {
        ...
        if(isContains(coordinate, leftBottom, midPoint))
        {
            maxLat = (minLat + maxLat) / 2;
            maxLng = (minLng + maxLng) / 2;
        }
        else if(isContains(coordinate, rightBottom, midPoint))
        {
            maxLat = (minLat + maxLat) / 2;
            minLng = (minLng + maxLng) / 2;
        }
        else if(isContains(coordinate, leftUp, midPoint))
        {
